...
# Using enum for EC_CURVE_REGISTRY


class EC_CURVE_REGISTRY(enum.Enum):
    brainpoolP160r1 = {
...
def get_curve(name: str):
    curve_params = {}
    for ec_enum in EC_CURVE_REGISTRY:
        if name.lower() == ec_enum.name.lower():
            curve_params = ec_enum
    if curve_params == {}:
        raise ValueError("Unknown elliptic curve name")
    return get_curve_safe(curve_params)
